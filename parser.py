...
class Function:
    ...
    def eval(self, module, external_funcs):
        ...
        if self.vdecls:
            for vdecl in self.vdecls.vars:
                types.append(vdecl.typename.value)
                names.append(vdecl.var)
        ...
        for i in range(len(names)):
            ...
            # varid_symbol_ptr_table[func.args[i].name] = func.args[i]
            if ir_arg_type[i].is_pointer:
                ptr = func.args[i]
...
class Statements:
    ...
    def eval(self, module, builder, external_funcs):
        ...
        for stmt in self.stmts:
            returned = stmt.eval(module, builder, external_funcs) or returned
            if returned:
                return returned
        return returned
...
class ReturnStatement:
    def __init__(self, exp=None):
        self.exp = exp
        self.stmt_type = "return_statement"
    ...
    def eval(self, module, builder, external_funcs):
        i = self.exp.eval(module, builder)
        builder.ret(i)
        return True
...
class VDeclStatement:
    def __init__(self, vdecl, exp):
        ...
        self.vdecl = vdecl
        self.exp = exp
        self.stmt_type = "vdecl_statement"
    ...
    def eval(self, module, builder, external_funcs):
        # alloc
        var_type = self.vdecl.typename.value
        var_name = self.vdecl.var.value
        is_ref = self.vdecl.is_ref
        # print(var_type)
        # print(var_name)
        if is_ref:
            if 'int' in var_type:
        ...
        if var_ptr.type.pointee == ir.IntType(32):
...
class ExpSemiStatement:
    def __init__(self, exp):
        self.exp = exp 
        self.stmt_type = "expsemi_statement"
...
class WhileStatement:
    def __init__(self, exp, stmt):
        self.exp = exp 
        self.stmt = stmt 
        self.stmt_type = "while_statement"
    ...
    def eval(self, module, builder, external_funcs):
        ...
        pred = self.exp.eval(module, builder)
        builder.cbranch(pred, w_body_block, w_after_block)
        # body
        builder.position_at_start(w_body_block)
        self.stmt.eval(module, builder, external_funcs)
        pred_body = self.exp.eval(module, builder)
        builder.cbranch(pred_body, w_body_block, w_after_block)
...
class IfStatement:
    ...
    def eval(self, module, builder, external_funcs):
        pred = self.exp.eval(module, builder)
        returned = False
        if pred.type == ir.FloatType():
            pred = builder.fptosi(pred, ir.IntType(1))
        if self.stmt2:
            with builder.if_else(pred) as (then, otherwise):
                with then:
                    returned_then = self.stmt1.eval(module, builder, external_funcs)
                with otherwise:
                    returned_otherwise = self.stmt1.eval(module, builder, external_funcs)
            returned = returned_then and returned_otherwise
        else:
            with builder.if_then(pred):
                self.stmt1.eval(module, builder, external_funcs)
        # not sure
        if returned:
            endif = builder.block
...
class PrintExpStatement:
    def __init__(self, exp):
        self.exp = exp 
        self.stmt_type = "printexp_statement"
...
class PrintSLitStatement:
    def __init__(self, slit):
        self.slit = slit 
        self.stmt_type = "printslit_statement"
