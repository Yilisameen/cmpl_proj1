...
_lr_productions = [
  ...
  ('PROG -> FUNCS','PROG',1,'p_prog','parser.py',452),
  ('PROG -> EXTERNS FUNCS','PROG',2,'p_prog','parser.py',453),
  ('EXTERNS -> EXTERNSTS','EXTERNS',1,'p_externs','parser.py',462),
  ('EXTERNS -> EXTERNS EXTERNSTS','EXTERNS',2,'p_externs','parser.py',463),
  ('FUNCS -> FUNC','FUNCS',1,'p_funcs','parser.py',474),
  ('FUNCS -> FUNCS FUNC','FUNCS',2,'p_funcs','parser.py',475),
  ('EXTERNSTS -> EXTERN TYPE ID LPAREN RPAREN SEMICOLON','EXTERNSTS',6,'p_extern','parser.py',485),
  ('EXTERNSTS -> EXTERN TYPE ID LPAREN TDECLS RPAREN SEMICOLON','EXTERNSTS',7,'p_extern','parser.py',486),
  ('FUNC -> DEF TYPE ID LPAREN RPAREN BLK','FUNC',6,'p_func','parser.py',495),
  ('FUNC -> DEF TYPE ID LPAREN VDECLS RPAREN BLK','FUNC',7,'p_func','parser.py',496),
  ('BLK -> LBRACE RBRACE','BLK',2,'p_blk','parser.py',505),
  ('BLK -> LBRACE STMTS RBRACE','BLK',3,'p_blk','parser.py',506),
  ('STMTS -> STMT','STMTS',1,'p_stmts','parser.py',516),
  ('STMTS -> STMTS STMT','STMTS',2,'p_stmts','parser.py',517),
  ('STMT -> BLK','STMT',1,'p_stmt_blk','parser.py',526),
  ('STMT -> RETURN SEMICOLON','STMT',2,'p_stmt_ret','parser.py',531),
  ('STMT -> RETURN EXP SEMICOLON','STMT',3,'p_stmt_ret','parser.py',532),
  ('STMT -> VDECL EQUAL EXP SEMICOLON','STMT',4,'p_stmt_vdecl','parser.py',541),
  ('STMT -> EXP SEMICOLON','STMT',2,'p_stmt_expSemi','parser.py',548),
  ('STMT -> WHILE LPAREN EXP RPAREN STMT','STMT',5,'p_stmt_while','parser.py',554),
  ('STMT -> IF LPAREN EXP RPAREN STMT','STMT',5,'p_stmt_if','parser.py',560),
  ('STMT -> IF LPAREN EXP RPAREN STMT ELSE STMT','STMT',7,'p_stmt_if','parser.py',561),
  ('STMT -> PRINT EXP SEMICOLON','STMT',3,'p_stmt_print_exp','parser.py',570),
  ('STMT -> PRINT SLIT SEMICOLON','STMT',3,'p_stmt_print_slit','parser.py',576),
  ('EXPS -> EXP','EXPS',1,'p_exps','parser.py',581),
  ('EXPS -> EXPS COMMA EXP','EXPS',3,'p_exps','parser.py',582),
  ('EXP -> EXPPAREN','EXP',1,'p_exp','parser.py',590),
  ('EXP -> BINOP','EXP',1,'p_exp','parser.py',591),
  ('EXP -> UOP','EXP',1,'p_exp','parser.py',592),
  ('EXP -> LIT','EXP',1,'p_exp','parser.py',593),
  ('EXP -> VARID','EXP',1,'p_exp','parser.py',594),
  ('EXP -> EXPGLOBID','EXP',1,'p_exp','parser.py',595),
  ('EXPPAREN -> LPAREN EXP RPAREN','EXPPAREN',3,'p_expParen','parser.py',599),
  ('EXPGLOBID -> GLOBID EXPWRAPPER','EXPGLOBID',2,'p_expGlobid','parser.py',603),
  ('EXPWRAPPER -> LPAREN RPAREN','EXPWRAPPER',2,'p_expWrapper','parser.py',607),
  ('EXPWRAPPER -> LPAREN EXPS RPAREN','EXPWRAPPER',3,'p_expWrapper','parser.py',608),
  ('ASSIGN -> VARID EQUAL EXP','ASSIGN',3,'p_assign','parser.py',615),
  ('TYPECAST -> LBRACKET TYPE RBRACKET EXP','TYPECAST',4,'p_typeCast','parser.py',619),
  ('BINOP -> ARITHOPS','BINOP',1,'p_binop','parser.py',623),
  ('BINOP -> LOGICOPS','BINOP',1,'p_binop','parser.py',624),
  ('BINOP -> ASSIGN','BINOP',1,'p_binop','parser.py',625),
  ('BINOP -> TYPECAST','BINOP',1,'p_binop','parser.py',626),
  ('ARITHOPS -> EXP TIMES EXP','ARITHOPS',3,'p_arithOps','parser.py',630),
  ('ARITHOPS -> EXP DIVIDE EXP','ARITHOPS',3,'p_arithOps','parser.py',631),
  ('ARITHOPS -> EXP PLUS EXP','ARITHOPS',3,'p_arithOps','parser.py',632),
  ('ARITHOPS -> EXP MINUS EXP','ARITHOPS',3,'p_arithOps','parser.py',633),
  ('LOGICOPS -> EXP EQUALITY EXP','LOGICOPS',3,'p_logicOps','parser.py',644),
  ('LOGICOPS -> EXP LESS EXP','LOGICOPS',3,'p_logicOps','parser.py',645),
  ('LOGICOPS -> EXP GREATER EXP','LOGICOPS',3,'p_logicOps','parser.py',646),
  ('LOGICOPS -> EXP AND EXP','LOGICOPS',3,'p_logicOps','parser.py',647),
  ('LOGICOPS -> EXP OR EXP','LOGICOPS',3,'p_logicOps','parser.py',648),
  ('UOP -> NOT EXP','UOP',2,'p_uop','parser.py',661),
  ('UOP -> MINUS EXP','UOP',2,'p_uop','parser.py',662),
  ('LIT -> TRUE','LIT',1,'p_lit','parser.py',669),
  ('LIT -> FALSE','LIT',1,'p_lit','parser.py',670),
  ('LIT -> NUMBER','LIT',1,'p_lit','parser.py',671),
  ('TDECLS -> TYPE','TDECLS',1,'p_tdecls','parser.py',675),
  ('TDECLS -> TDECLS COMMA TYPE','TDECLS',3,'p_tdecls','parser.py',676),
  ('VDECLS -> VDECLS COMMA VDECL','VDECLS',3,'p_vdecls','parser.py',684),
  ('VDECLS -> VDECL','VDECLS',1,'p_vdecls','parser.py',685),
  ('VDECL -> TYPE VARID','VDECL',2,'p_vdecl','parser.py',695),
  ('VARID -> DOLLAR ID','VARID',2,'p_varid','parser.py',700),
  ('TYPE -> INT','TYPE',1,'p_simpleType','parser.py',705),
  ('TYPE -> FLOAT','TYPE',1,'p_simpleType','parser.py',706),
  ('TYPE -> CINT','TYPE',1,'p_simpleType','parser.py',707),
  ('TYPE -> VOID','TYPE',1,'p_simpleType','parser.py',708),
  ('TYPE -> REF TYPE','TYPE',2,'p_refType','parser.py',712),
  ('TYPE -> NOALIAS REF TYPE','TYPE',3,'p_refTypeNoAlias','parser.py',717),
  ('GLOBID -> ID','GLOBID',1,'p_globid','parser.py',722),
]
