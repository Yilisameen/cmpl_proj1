...
_lr_productions = [
  ...
  ('PROG -> FUNCS','PROG',1,'p_prog','parser.py',474),
  ('PROG -> EXTERNS FUNCS','PROG',2,'p_prog','parser.py',475),
  ('EXTERNS -> EXTERNSTS','EXTERNS',1,'p_externs','parser.py',491),
  ('EXTERNS -> EXTERNS EXTERNSTS','EXTERNS',2,'p_externs','parser.py',492),
  ('FUNCS -> FUNC','FUNCS',1,'p_funcs','parser.py',503),
  ('FUNCS -> FUNCS FUNC','FUNCS',2,'p_funcs','parser.py',504),
  ('EXTERNSTS -> EXTERN TYPE ID LPAREN RPAREN SEMICOLON','EXTERNSTS',6,'p_extern','parser.py',514),
  ('EXTERNSTS -> EXTERN TYPE ID LPAREN TDECLS RPAREN SEMICOLON','EXTERNSTS',7,'p_extern','parser.py',515),
  ('FUNC -> DEF TYPE ID LPAREN RPAREN BLK','FUNC',6,'p_func','parser.py',525),
  ('FUNC -> DEF TYPE ID LPAREN VDECLS RPAREN BLK','FUNC',7,'p_func','parser.py',526),
  ('BLK -> LBRACE RBRACE','BLK',2,'p_blk','parser.py',564),
  ('BLK -> LBRACE STMTS RBRACE','BLK',3,'p_blk','parser.py',565),
  ('STMTS -> STMT','STMTS',1,'p_stmts','parser.py',575),
  ('STMTS -> STMTS STMT','STMTS',2,'p_stmts','parser.py',576),
  ('STMT -> BLK','STMT',1,'p_stmt_blk','parser.py',585),
  ('STMT -> RETURN SEMICOLON','STMT',2,'p_stmt_ret','parser.py',590),
  ('STMT -> RETURN EXP SEMICOLON','STMT',3,'p_stmt_ret','parser.py',591),
  ('STMT -> VDECL EQUAL EXP SEMICOLON','STMT',4,'p_stmt_vdecl','parser.py',600),
  ('STMT -> EXP SEMICOLON','STMT',2,'p_stmt_expSemi','parser.py',607),
  ('STMT -> WHILE LPAREN EXP RPAREN STMT','STMT',5,'p_stmt_while','parser.py',613),
  ('STMT -> IF LPAREN EXP RPAREN STMT','STMT',5,'p_stmt_if','parser.py',619),
  ('STMT -> IF LPAREN EXP RPAREN STMT ELSE STMT','STMT',7,'p_stmt_if','parser.py',620),
  ('STMT -> PRINT EXP SEMICOLON','STMT',3,'p_stmt_print_exp','parser.py',629),
  ('STMT -> PRINT SLIT SEMICOLON','STMT',3,'p_stmt_print_slit','parser.py',635),
  ('EXPS -> EXP','EXPS',1,'p_exps','parser.py',640),
  ('EXPS -> EXPS COMMA EXP','EXPS',3,'p_exps','parser.py',641),
  ('EXP -> EXPPAREN','EXP',1,'p_exp','parser.py',649),
  ('EXP -> BINOP','EXP',1,'p_exp','parser.py',650),
  ('EXP -> UOP','EXP',1,'p_exp','parser.py',651),
  ('EXP -> LIT','EXP',1,'p_exp','parser.py',652),
  ('EXP -> VARID','EXP',1,'p_exp','parser.py',653),
  ('EXP -> EXPGLOBID','EXP',1,'p_exp','parser.py',654),
  ('EXPPAREN -> LPAREN EXP RPAREN','EXPPAREN',3,'p_expParen','parser.py',658),
  ('EXPGLOBID -> GLOBID EXPWRAPPER','EXPGLOBID',2,'p_expGlobid','parser.py',662),
  ('EXPWRAPPER -> LPAREN RPAREN','EXPWRAPPER',2,'p_expWrapper','parser.py',666),
  ('EXPWRAPPER -> LPAREN EXPS RPAREN','EXPWRAPPER',3,'p_expWrapper','parser.py',667),
  ('ASSIGN -> VARID EQUAL EXP','ASSIGN',3,'p_assign','parser.py',674),
  ('TYPECAST -> LBRACKET TYPE RBRACKET EXP','TYPECAST',4,'p_typeCast','parser.py',678),
  ('BINOP -> ARITHOPS','BINOP',1,'p_binop','parser.py',682),
  ('BINOP -> LOGICOPS','BINOP',1,'p_binop','parser.py',683),
  ('BINOP -> ASSIGN','BINOP',1,'p_binop','parser.py',684),
  ('BINOP -> TYPECAST','BINOP',1,'p_binop','parser.py',685),
  ('ARITHOPS -> EXP TIMES EXP','ARITHOPS',3,'p_arithOps','parser.py',689),
  ('ARITHOPS -> EXP DIVIDE EXP','ARITHOPS',3,'p_arithOps','parser.py',690),
  ('ARITHOPS -> EXP PLUS EXP','ARITHOPS',3,'p_arithOps','parser.py',691),
  ('ARITHOPS -> EXP MINUS EXP','ARITHOPS',3,'p_arithOps','parser.py',692),
  ('LOGICOPS -> EXP EQUALITY EXP','LOGICOPS',3,'p_logicOps','parser.py',703),
  ('LOGICOPS -> EXP LESS EXP','LOGICOPS',3,'p_logicOps','parser.py',704),
  ('LOGICOPS -> EXP GREATER EXP','LOGICOPS',3,'p_logicOps','parser.py',705),
  ('LOGICOPS -> EXP AND EXP','LOGICOPS',3,'p_logicOps','parser.py',706),
  ('LOGICOPS -> EXP OR EXP','LOGICOPS',3,'p_logicOps','parser.py',707),
  ('UOP -> NOT EXP','UOP',2,'p_uop','parser.py',720),
  ('UOP -> MINUS EXP','UOP',2,'p_uop','parser.py',721),
  ('LIT -> TRUE','LIT',1,'p_lit','parser.py',728),
  ('LIT -> FALSE','LIT',1,'p_lit','parser.py',729),
  ('LIT -> NUMBER','LIT',1,'p_lit','parser.py',730),
  ('TDECLS -> TYPE','TDECLS',1,'p_tdecls','parser.py',734),
  ('TDECLS -> TDECLS COMMA TYPE','TDECLS',3,'p_tdecls','parser.py',735),
  ('VDECLS -> VDECLS COMMA VDECL','VDECLS',3,'p_vdecls','parser.py',743),
  ('VDECLS -> VDECL','VDECLS',1,'p_vdecls','parser.py',744),
  ('VDECL -> TYPE VARID','VDECL',2,'p_vdecl','parser.py',754),
  ('VARID -> DOLLAR ID','VARID',2,'p_varid','parser.py',759),
  ('TYPE -> INT','TYPE',1,'p_simpleType','parser.py',764),
  ('TYPE -> FLOAT','TYPE',1,'p_simpleType','parser.py',765),
  ('TYPE -> CINT','TYPE',1,'p_simpleType','parser.py',766),
  ('TYPE -> VOID','TYPE',1,'p_simpleType','parser.py',767),
  ('TYPE -> REF TYPE','TYPE',2,'p_refType','parser.py',771),
  ('TYPE -> NOALIAS REF TYPE','TYPE',3,'p_refTypeNoAlias','parser.py',788),
  ('GLOBID -> ID','GLOBID',1,'p_globid','parser.py',805),
]
